...
```{r}
models <- c("sex_only", "sex_tail", "sex_tail2", 
...
for (k in 1:length(models)){
  ...
  if (!file.exists(paste0("RData/LO_", models[[k]], ".rds"))){
    ...
    saveRDS(jm.out, file = paste0("RData/LO_", models[[k]], ".rds"))
  } else {
    jm.out <- readRDS(file = paste0("RData/LO_", models[[k]], ".rds"))
    
  }
  DIC[k] <- jm.out$jm.out$DIC
  if (!file.exists(paste0("RData/LO_", models[[k]], "_loo_out.rds"))){
    loo.out <- compute.LOOIC(loglik = jm.out$jm.out$sims.list$loglik,
                             data.vector = as.vector(jm.out$data$H1),
                             MCMC.params = jm.out$params)
    
    saveRDS(loo.out, paste0("RData/LO_", models[[k]], "_loo_out.rds") ) 
  } else {
    loo.out <- readRDS(paste0("RData/LO_", models[[k]], "_loo_out.rds"))
  }
...
Find the ID of somehwat high p(male)
