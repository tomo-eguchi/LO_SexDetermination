...
dat.1 %>% select(ID, tail_1, tail_2, log_Testo1_mean0, log_Testo2_mean0, pMale) %>%
  mutate(LabID = ID) %>%  select(-ID) %>% right_join(dat.0, by = "LabID") -> dat.2.ptail
...
dat.2 %>% mutate(Estimated_Sex = ifelse(pMale > 0.5, "Male", "Female")) %>% 
  group_by(Estimated_Sex) %>% summarize(n = n(), ratio = n/nrow(dat.2)) -> Sex.ratio
