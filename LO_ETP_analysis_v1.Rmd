...
dat.0 <- read_csv(file = "data/EPacLO_2021-07-02.csv",
                  col_types = col_def)
...
The sex_tail_SCL model is best.  We should also use Pareto k statistic to see if the model fit right. 

```{r}
best.model.name <- "sex_tail_SCL"
best.model <- readRDS(paste0("RData/LO_", best.model.name, ".rds"))
dat.1$Model <- "sex_tail_SCL"
...
Only 3 (maybe 2) data points showed high Pareto k values (> 0.7) whereas others were all < 0.7. So, I'll just go with this model. Look at the results:
...
#
if (!file.exists(paste0("data/estimated_sex_LO_ETP_", Sys.Date(), ".csv")))
  write.csv(dat.2,
...
```{r}
loo.out$loo.out$pointwise %>% data.frame() %>%
...
Turns out variance between two readings was not the problem. 
