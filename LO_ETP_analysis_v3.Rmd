...
file.name <- "data/EPac_LO_13June2022.csv"
dat.0 <- read_csv(file = file.name,
                  col_types = col_def)

file.name.2 <- "data/EPac_LO_TH20_18Feb2022.csv"
dat.0.2 <- read_csv(file = file.name.2,
                  col_types = col_def_2) %>% 
  select(LabID, TH20_C)

dat.0 %>% left_join(dat.0.2, by = "LabID") -> dat.0
summary(dat.0)
```
...
write_csv(LOOIC_DIC, file = "data/model_comp_June2022.csv")

```


```{r}
best.model.name <- LOOIC_DIC[1, "Model"] # "sex_ptail_SCL_temp"
best.model <- readRDS(paste0("RData/LO_", 
                             best.model.name, "_June2022.rds"))
...
second.model <- readRDS(paste0("RData/LO_", 
                             second.model.name, "_June2022.rds"))
...
third.model <- readRDS(paste0("RData/LO_", 
                             third.model.name, "_June2022.rds"))
...
Although the sex_ptail_SCL_temp and sex_tail_SCL models are the first and second best. The second model (sex_tail_SCL) has fewer (3) bad Pareto k diagnostic values (>0.5) than the best model (sex_ptail_SCL_temp, 5). So, I use the second best model. Look at the results:
...
dat.1[high_pareto_k$rowID, 
      c("ID", "Mean.testo", "log_Testo_mean", "SCL", "tail_length")]
```
...
ggplot(data = dat.1) +
  ...
  geom_point(data = dat.1 %>% filter(ID == 65937 | ID == 65946),
